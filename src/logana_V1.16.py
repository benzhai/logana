...
VERSION="LOGANA V1.16, 2018/2/27; copyright by less (less@beescast.com)"
...
def src_domain_cnt(ustr, rstr):
    ...
    src_cnt(u_host, r_host)

def  dump_src_topx(D_TAB, utopx, rtopx, ofile):
    UIDX = 0
    UITEMS = sorted(D_TAB.items(), key=lambda TU: TU[1], reverse=True)
    print "\n ----------------------------------------------------------------------------------------"
    print >> ofile, "\n ----------------------------------------------------------------------------------------"
    print " %s:" % ("Source")
        ...
        if kdicts != None:
            ...
            for kdict in kdicts:
                if kdict.has_key('di') and uri_compare(kdict['di'], DUF) == True:
                    if RUF.strip() != '' and uri_compare(kdict['di'].split('\.')[1],RUF) != True:
                        KHIT = 1
                        break
...
def main():
    ...
    if options.ddtopx > 0:
        dump_tab_topx("Dest Domain",  DD_TAB, options.ddtopx, ofile)

    if options.dutopx > 0:
        dump_tab_topx("Dest url",     DU_TAB, options.dutopx, ofile)

    if options.rdtopx > 0:
        dump_tab_topx("Refer Domain", RD_TAB, options.rdtopx, ofile)

    if options.rutopx > 0:
        dump_tab_topx("Refer Domain",  RU_TAB, options.rutopx, ofile)
    ...
    if options.sdtopx > 0:
        dump_src_topx(DD_TAB,options.ddtopx, options.sdtopx, ofile)

    if options.sutopx > 0:
        dump_src_topx(DU_TAB,options.dutopx, options.sutopx, ofile)
