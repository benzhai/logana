...
def log_filter(ifile, ofile, kdict, options):
    global CNT, HIT, END
    ...
    for LINE in ifile:
        ...
        if kdict != None and kdict.has_key('du'):
            if field_compare(kdict['du'], DUF) == False:
                continue
                
        #print "ru=%s, RUF=%s" %(kdict['ru'], RUF)
        if kdict != None and kdict.has_key('ru'):
            if field_compare(kdict['ru'], RUF) == False:
                continue

        if kdict != None and kdict.has_key('ua'):
...
def logs_filter(ofile, options):
    ...
    for ipath in ipaths:
        ...
        log_filter(ifile, ofile, kdict, options)
...
def main():
    ...
    t = threading.Thread(target=logs_filter, args=(ofile, options, ))
    t.start()
